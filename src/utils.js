...
export function calcAvailability(zip, reservations = []) {
  ...
  if (!sc) return { base, best15: 0, best30: 0, best45: 0, ring15: [], ring30: [], ring45: [], reserved: totalReserved }
    ...
    if (de) {
      ...
      for (let m = 2; m >= 0; m--) {
        if (leads[m] !== null && pct[m] !== null) {
          if (pct[m] > 1.0) {
            odExcess = leads[m] - (de[5] || 0)
            odMonth = ['Dec','Jan','Feb'][m]
            odPct = pct[m]
          }
          break
        }
